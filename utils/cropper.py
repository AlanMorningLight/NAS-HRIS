...
class Cropper(object):
    def __init__(self, args, predict=None):
        ...
        self.output_label_path = os.path.join(self.output_path, 'label')

        if predict:
            self.ensure_and_mkdir(self.output_path)
            self.ensure_and_mkdir(self.output_image_path)
        else:
            self.ensure_and_mkdir(self.output_path)
            self.ensure_and_mkdir(self.output_image_path)
            self.ensure_and_mkdir(self.output_label_path)
        # by default
        self.size_x = self.args.cropped_size
        self.size_y = self.args.cropped_size
    ...
    def image_processor(self, image_path, label_path):
        ...
        self.save_images(patches, self.output_image_path, filename)
        ...
        if self.predict is False:
            ...
            self.save_images(label_patches, self.output_label_path, label_filename)
